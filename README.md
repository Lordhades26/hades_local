...
### 4. Configurar HADES (automático desde v1.2.0)

No hace falta editar nada. HADES **autodetecta** dónde está Ollama
probando en orden: `HADES_OLLAMA_URL`/`OLLAMA_HOST` → `127.0.0.1`
→ nameserver de `/etc/resolv.conf` (WSL) → `host.docker.internal`
→ gateway por defecto (VirtualBox/VMware NAT) → `10.0.2.2`.
Usa la primera URL que responda y detecta el modelo solo.

Solo si Ollama está en otra máquina o puerto, fuérzalo:
```bash
...
# HADES detecta la IP de Windows automáticamente (v1.2.0+).
# Solo verifica:
python3 hades_local.py status
```
...
Para eliminar HADES del equipo de forma limpia:

```bash
cd ~/hades_local        # o donde lo hayas clonado
bash uninstall.sh            # interactivo
bash uninstall.sh --yes      # borra el agente (conserva reportes y Ollama)
bash uninstall.sh --purge    # borra TODO (agente + reportes + modelos + Ollama)
```
...
### v1.2.0
- Autodetección de Ollama: ya no hay que editar `OLLAMA_URL`. Funciona en Linux nativo, WSL2 y VMs (VirtualBox/VMware NAT). Override con `HADES_OLLAMA_URL`.
- Nuevo `uninstall.sh` (modos `--yes` / `--purge`).
- `build_pendrive.sh` pasa la URL por variable de entorno.
